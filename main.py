...
async def cmd_ref(m: Message):
    ...
    text = (
        ...
        "Пригласите друга по вашей ссылке. Как только он начнёт матч — вы получаете бонус.\n\n"
        f"🔗 Ваша ссылка:\n{link}\n\n"
...
async def cmd_withdraw(m: Message):
    ...
    text = (
        ...
        "💎 Уважаемые игроки!\n\n"
        "За выводом средств обращайтесь: @Arenatransfers\n\n"
    )
...
async def cb_rules(cq: CallbackQuery):
    ...
    await edit_or_send(
        ...
        "• Режим 1 — 🎰 777:\n"
        "7-7-7 — Победа!\n"
        "Bar-Bar-Bar — проигрыш бросившего!\n"
        "🍋🍋🍋 — ничья!\n\n"
        "• Режим 2 — 🎲: каждый делает 3 броска; больше сумма — победа; равенство — ничья!\n\n"
...
async def cb_stake(cq: CallbackQuery):
    ...
    if opp:
        ...
        if db.can_start(match_id):
            ...
            await bot.send_message(uid, "🧾 Счёт выставлен. Ждем оплату обеих сторон.")
            await bot.send_message(opp, "🧾 Счёт выставлен. Ждем оплату обеих сторон.")
    else:
...
async def cmd_claim(m: Message):
    """
    Игрок запрашивает техническую победу в режиме 🎲 3 броска,
    если у него уже 3/3 бросков, а у соперника < 3/3.
    Подходит для ситуации, когда соперник исчез/заблокировал бота.
    """
    uid = m.from_user.id
    mv = row_to_match(db.get_match_by_user(uid))

    if not mv or mv.winner_id:
        return await m.answer("Матч не найден или уже завершён. /join")

    if not mv.active:
        return await m.answer("Матч ещё не стартовал — заявка на победу невозможна.")

    if mv.game_mode != "dice3":
        return await m.answer("Команда доступна только в режиме 🎲 3 броска.")

    # текущее состояние бросков
    st = db.get_dice_state(mv.id)
    if not st:
        return await m.answer("Не удалось получить состояние матча. Попробуй позже.")

    slot = 1 if uid == mv.p1_id else 2
    my_cnt = int(st["p1_dice_cnt"] if slot == 1 else st["p2_dice_cnt"])
    opp_cnt = int(st["p2_dice_cnt"] if slot == 1 else st["p1_dice_cnt"])

    # Условие техпобеды: у тебя 3/3, у соперника < 3/3
    if my_cnt >= 3 and opp_cnt < 3:
        await on_win(uid, mv)
        return await m.answer("✅ Техническая победа засчитана: у тебя 3/3 бросков, соперник не завершил.")
    elif my_cnt < 3 and opp_cnt >= 3:
        return await m.answer("Техпобеда невозможна: у соперника уже 3/3, а у тебя нет.")
    elif my_cnt >= 3 and opp_cnt >= 3:
        return await m.answer("Оба завершили 3/3 бросков — подведи итог броском соперника или дождись автозавершения.")
    else:
